/*
  # Insert test data for Patreon integration

  1. Test Data
    - Creates a sample Patreon page
    - Adds a campaign with tiers
    - Adds sample patrons and memberships
    - All data is connected to the test user's account
...
DECLARE
  ...
  tier_ids text[] := ARRAY['1', '2', '3']::text[];
BEGIN
  -- Insert test Patreon page
  INSERT INTO public.patreon_pages (id, user_id, name, client_id, client_secret)
  VALUES (
    gen_random_uuid(),
    user_id,
    'Test Creator Page',
  ...
  FOR i IN 1..array_length(patron_ids, 1) LOOP
    ...
    VALUES (
      ...
      (random() * 10000)::bigint,
      CASE 
        WHEN i <= 15 THEN ARRAY[tier_ids[1]]::uuid[]
        WHEN i <= 22 THEN ARRAY[tier_ids[1], tier_ids[2]]::uuid[]
        ELSE ARRAY[tier_ids[1], tier_ids[2], tier_ids[3]]::uuid[]
      END,
